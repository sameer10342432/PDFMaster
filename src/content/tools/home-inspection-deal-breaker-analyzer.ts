...
❌ **"Home sold as-is"** (after accepting your offer)
- They knew about problems
- Not willing to negotiate fairly
- Consider walking

❌ **Won't provide receipts for claimed repairs**
...
❌ **Refuses all reasonable requests**
- Even safety items
...
### Proceed If:
✅ No safety hazards or seller addresses them
✅ Major systems functional or seller provides credit
✅ Total repairs < 5-7% of purchase price
✅ Seller negotiates reasonably
✅ Still good deal after repairs
✅ You have cash reserves for unexpected

### Walk Away If:
❌ Undisclosed major defects (dishonesty)
❌ Multiple deal-breaker issues
❌ Repairs > 10% of purchase price
❌ Safety hazards seller won't fix
❌ Gut feeling says no
❌ Seller completely unreasonable
❌ You can't afford repairs + purchase

**Remember**: There will always be another house. Don't let emotional attachment or sunk costs force you into a bad decision. A home inspection is your protection—use it wisely!
    `,
  },
  calculator: {
    fields: [
    ...
    calculate: (values) => {
      ...
      if (dealBreakerScore >= 70) {
        scoreDescription = `${dealBreakerScore.toFixed(0)}/100 - ❌ CRITICAL - Walk away recommended`;
      } else if (dealBreakerScore >= 50) {
        scoreDescription = `${dealBreakerScore.toFixed(0)}/100 - ⚠️ SEVERE - Serious concerns, negotiate hard or walk`;
      } else if (dealBreakerScore >= 30) {
        scoreDescription = `${dealBreakerScore.toFixed(0)}/100 - ⚠️ MODERATE - Negotiate significant repairs/credits`;
      } else if (dealBreakerScore >= 15) {
        scoreDescription = `${dealBreakerScore.toFixed(0)}/100 - ⚠️ MINOR - Negotiate but likely acceptable`;
      } else {
        scoreDescription = `${dealBreakerScore.toFixed(0)}/100 - ✅ LOW - Proceed with minor negotiations`;
      }

      let recommendation = "";
      
      if (structural > 20000) {
        recommendation = `❌ WALK AWAY\n\nMajor structural issues ($${structural.toLocaleString()}) are severe.\n\n• These are expensive and risky repairs\n• Could indicate deeper problems\n• Will impact future resale\n• Hard to get accurate estimates\n\n💡 Unless seller reduces price by $${(totalRepairs * 1.5).toLocaleString()}+ or repairs completely before closing, walk away.`;
      } else if (water > 15000 || dealBreakerScore >= 70) {
        recommendation = `❌ WALK AWAY\n\nToo many serious issues (Score: ${dealBreakerScore.toFixed(0)}/100).\n\n• Total repairs: $${totalRepairs.toLocaleString()}\n• ${repairPercent.toFixed(1)}% of purchase price\n• Net equity after repairs: $${netEquity.toLocaleString()}\n• Risk of hidden issues too high\n\n💡 Find a better property. This one has too many red flags.`;
      } else if (repairPercent > 12 || dealBreakerScore >= 50) {
        recommendation = `⚠️ NEGOTIATE HARD OR WALK\n\nSignificant issues requiring tough negotiation.\n\n• Total repairs: $${totalRepairs.toLocaleString()} (${repairPercent.toFixed(1)}% of price)\n• Request full credit or price reduction\n• If seller refuses, seriously consider walking\n• Have backup plans ready\n\n💡 Only proceed if:\n- Seller provides $${totalRepairs.toLocaleString()}+ credit\n- Still a good deal after repairs\n- You have cash reserves for unknowns`;
      } else if (repairPercent > 5) {
        recommendation = `⚠️ NEGOTIATE SIGNIFICANT REPAIRS\n\nModerate issues worth addressing.\n\n• Total repairs: $${totalRepairs.toLocaleString()} (${repairPercent.toFixed(1)}% of price)\n• Net equity after repairs: $${netEquity.toLocaleString()}\n• Negotiate ${Math.round(totalRepairs * 0.75).toLocaleString()} - $${totalRepairs.toLocaleString()} credit\n\n💡 Prioritize:\n1. Safety hazards ($${safety.toLocaleString()}) - must fix\n2. Major systems ($${systems.toLocaleString()}) - negotiate hard\n3. Deferred maintenance ($${maintenance.toLocaleString()}) - optional`;
      } else {
        recommendation = `✅ PROCEED WITH MINOR NEGOTIATIONS\n\nReasonable issues for a used home.\n\n• Total repairs: $${totalRepairs.toLocaleString()} (${repairPercent.toFixed(1)}% of price)\n• Net equity after repairs: $${netEquity.toLocaleString()}\n• This is manageable\n\n💡 Negotiate:\n- Focus on significant items ($1,000+ each)\n- Request $${Math.round(totalRepairs * 0.6).toLocaleString()} - $${Math.round(totalRepairs * 0.8).toLocaleString()} credit\n- Let go of cosmetic items\n- Proceed with purchase`;
      }
...
${structural > 0 ? `• Structural issues: $${structural.toLocaleString()} - Seller repairs or credit` : ''}
${water > 0 ? `• Water damage/mold: $${water.toLocaleString()} - Seller repairs before closing` : ''}
${safety > 0 ? `• Safety hazards: $${safety.toLocaleString()} - Must be fixed before closing` : ''}
${structural === 0 && water === 0 && safety === 0 ? '• None - no critical safety issues' : ''}

Priority 2 - NEGOTIATE HARD (Major items):
${systems > 0 ? `• Major systems: $${systems.toLocaleString()} - Request credit or price reduction` : '• None'}

Priority 3 - NICE TO GET (If seller agrees):
${maintenance > 0 ? `• Deferred maintenance: $${maintenance.toLocaleString()} - Request $${Math.round(maintenance * 0.5).toLocaleString()} credit` : '• None'}
