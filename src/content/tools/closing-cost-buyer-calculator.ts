...
📋 Loan-Related Fees: $${(loanFees + appraisal + creditReport).toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Loan origination: $${loanFees.toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Appraisal: $${appraisal}
  • Credit report: $${creditReport}

🏛️ Title & Escrow: $${(titleInsurance + escrowFee).toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Title insurance: $${titleInsurance.toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Escrow/settlement: $${escrowFee.toLocaleString(undefined, {maximumFractionDigits: 0})}

📝 Government Fees: $${(recordingFees + transferTax).toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Recording fees: $${recordingFees.toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Transfer taxes: $${transferTax.toLocaleString(undefined, {maximumFractionDigits: 0})}

💵 Prepaids: $${(homeownersIns + propertyTaxEscrow).toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Insurance (1 year): $${homeownersIns.toLocaleString(undefined, {maximumFractionDigits: 0})}
  • Property tax escrow: $${propertyTaxEscrow.toLocaleString(undefined, {maximumFractionDigits: 0})}
...
✅ Shop Multiple Lenders:
  ...
  • Negotiate origination fees (save $500-2,000)
  • Ask for lender credits

✅ Ask for Seller Concessions:
  • ${loanType === "conventional" && downPct >= 0.25 ? 'Up to 9% allowed' : 
       loanType === "conventional" && downPct >= 0.1 ? 'Up to 6% allowed' :
       loanType === "fha" ? 'Up to 6% allowed' :
       loanType === "va" ? 'Up to 4% allowed' : 'Up to 6% allowed'}
  • Increase offer, ask seller to cover closing costs
  • Could save $${Math.round(Math.min(totalClosingCosts, price * 0.06)).toLocaleString()}

✅ Close at Month End:
  • Less prepaid interest
  • Could save $${Math.round(loanAmount * 0.07 / 12 * 0.5).toLocaleString()}

✅ Shop Title Insurance:
  • Not all states allow, but check
  • Could save $200-500

${downPct < 0.2 ? '\n⚠️ Consider 20% Down:\n  • Eliminates PMI requirement\n  • Saves $' + Math.round(loanAmount * 0.01 / 12).toLocaleString() + '/month\n  • Better loan terms' : ''}

✅ Use First-Time Buyer Programs:
  • State/local assistance programs
  • Grant programs for closing costs
  • Could cover $1,000-5,000+
      `;
