...
✅ Job relocation
✅ Can afford to move up
✅ Spring/summer in your market

### Bad Times to Sell:
❌ Underwater on mortgage
❌ Owned < 2 years (capital gains)
❌ Market declining
❌ Winter (in seasonal markets)
❌ Need to sell immediately (weak position)
❌ Can't afford transaction costs
...
📉 Deductions:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🏠 Mortgage Payoff: -$${mortgage.toLocaleString()}

💼 Real Estate Commission (${(commPct * 100).toFixed(1)}%): -$${commission.toLocaleString()}
  • Listing agent: -$${(commission / 2).toLocaleString()}
  • Buyer's agent: -$${(commission / 2).toLocaleString()}

🏛️ Title & Escrow: -$${titleEscrow.toLocaleString()}

📝 Transfer Taxes (${(taxRate * 100).toFixed(2)}%): -$${transferTax.toLocaleString()}

💚 Seller Concessions: -$${concessions.toLocaleString()}

🔧 Repairs/Improvements: -$${repairCosts.toLocaleString()}

📋 Other Fees: -$${otherFees.toLocaleString()}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

💵 NET PROCEEDS: $${netProceeds.toLocaleString()} (${netPercent.toFixed(1)}%)
      `;

      let recommendations = "";
      
      if (netProceeds < 0) {
        recommendations = `❌ SHORT SALE SITUATION\n\nYou owe MORE than the home will sell for!\n\n• Shortfall: $${Math.abs(netProceeds).toLocaleString()}\n• You'd need to bring this cash to closing\n• Or pursue short sale (bank accepts less)\n• Or wait to sell until more equity\n\n💡 Options:\n1. Wait and pay down mortgage\n2. Make improvements to increase value\n3. Short sale (impacts credit)\n4. Refinance if possible\n5. Rent it out instead`;
      } else if (netPercent < 5) {
        recommendations = `⚠️ MINIMAL PROCEEDS\n\nYou'll only keep ${netPercent.toFixed(1)}% of sale price.\n\n• Very little profit after costs\n• Consider if selling makes sense now\n• Might be better to wait and build equity\n\n💡 Ways to Improve:\n• Negotiate lower commission (try 5% instead of ${(commPct * 100)}%)\n• Reduce/eliminate pre-sale repairs\n• Minimize seller concessions\n• Price higher if market allows\n• Wait until you have more equity`;
      } else if (netPercent < 20) {
        recommendations = `⚠️ MODEST PROCEEDS\n\nYou'll keep ${netPercent.toFixed(1)}% of sale price.\n\n• Net proceeds: $${netProceeds.toLocaleString()}\n• Should cover moving and transition\n• But limited funds for upgrade\n\n💡 Optimization Tips:\n• Negotiate commission down to 5% (save $${((sale * commPct) - (sale * 0.05)).toLocaleString()})\n• Limit concessions to negotiated items only\n• Strategic repairs (ROI focused)\n• Close at month start to reduce prorated costs`;
      } else if (netPercent < 40) {
        recommendations = `✅ GOOD PROCEEDS\n\nYou'll keep ${netPercent.toFixed(1)}% of sale price.\n\n• Net proceeds: $${netProceeds.toLocaleString()}\n• Solid amount for next home or other goals\n• Reasonable transaction\n\n💡 Optimization Tips:\n• Still try to negotiate commission to 5% (save $${((sale * commPct) - (sale * 0.05)).toLocaleString()})\n• Get multiple agent quotes\n• Strategic staging for higher sale price\n• Time sale for spring/summer if possible`;
      } else {
        recommendations = `✅ EXCELLENT PROCEEDS\n\nYou'll keep ${netPercent.toFixed(1)}% of sale price!\n\n• Net proceeds: $${netProceeds.toLocaleString()}\n• Significant equity position\n• Great foundation for next move\n\n💡 Planning Tips:\n• Consider capital gains tax (exclusion: $250k single, $500k married)\n• Plan use of proceeds (down payment, investment, etc)\n• Timing: Spring/summer typically best\n• Professional presentation maximizes price`;
      }

      // Commission savings suggestion
      const commissionSavings = commission - (sale * 0.05);
      if (commissionSavings > 1000) {
        recommendations += `\n\n💰 COMMISSION SAVINGS:\n• If you negotiate 5% instead of ${(commPct * 100)}%: Save $${commissionSavings.toLocaleString()}\n• Interview multiple agents\n• Consider flat-fee or discount brokers\n• Everything is negotiable!`;
      }
